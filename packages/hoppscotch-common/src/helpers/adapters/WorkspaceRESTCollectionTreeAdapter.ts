import { ChildrenResult, SmartTreeAdapter } from "@hoppscotch/ui/dist/src/helpers/treeAdapter"
import { Ref, ref, watchEffect } from "vue"
import { NewWorkspaceService } from "~/services/new-workspace";
import { HandleRef } from "~/services/new-workspace/handle";
import { RESTCollectionViewCollection, RESTCollectionViewRequest } from "~/services/new-workspace/view"
...
export class WorkspaceRESTCollectionTreeAdapter implements SmartTreeAdapter<WorkspaceRESTCollectionListNode> {
  ...
  public getChildren(nodeID: string | null): Ref<ChildrenResult<WorkspaceRESTCollectionListNode>> {
    if (this.workspaceHandle.value.type !== "ok") {
      throw new Error("Cannot issue children with invalid workspace handle")
    }

    const result = ref<ChildrenResult<WorkspaceRESTCollectionListNode>>({ status: "loading" });

    if (nodeID !== null) {
      (async () => {
        const collectionHandleResult = await this.workspaceService.getCollectionHandle(this.workspaceHandle, nodeID)
        ...
        const collectionChildrenResult = await this.workspaceService.getRESTCollectionChildrenView(collectionHandle)
      ...
      (async () => {
        const viewResult = await this.workspaceService.getRESTRootCollectionView(this.workspaceHandle);
        ...
        watchEffect(
          () => {
            ...
            if (viewHandle.value.data.loading.value) {
              ...
              result.value = {
                status: "loaded",
                data: viewHandle.value.data.collections.value.map(
                  (coll) => ({
