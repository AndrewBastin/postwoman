...
const treeAdapter = markRaw(new WorkspaceRESTCollectionTreeAdapter(props.workspaceHandle, workspaceService))
...
async function addNewRootCollection(name: string) {
  ...
  await workspaceService.createRESTRootCollection(props.workspaceHandle, name)
